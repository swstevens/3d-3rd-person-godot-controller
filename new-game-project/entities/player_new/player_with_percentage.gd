...
@export var min_jump_force: float = 12
@export var max_jump_force: float = 16
...
func handle_jumping():
	if Input.is_action_just_pressed("ui_accept"):
		if current_state == FLOOR:
			...
			velocity.y = calculate_jump_force(horizontal_speed)
			is_jump_held = true
			
			# Small momentum boost for maintaining speed
			if horizontal_speed > 0.1:
				var speed_percentage = clamp(horizontal_speed / max_run_speed, 0.0, 1.0)
				var momentum_boost = 1.05 + (speed_percentage * 0.1)
			...
			velocity.y += min_jump_force * 0.7  # Add some vertical boost
			wall_jump_lock = false

func calculate_jump_force(horizontal_speed:float) -> float:
	var speed_percentage = clamp(horizontal_speed / max_run_speed, 0.0, 1.0)
	
	# Interpolate between min and max jump force based on speed percentage
	return lerp(min_jump_force, max_jump_force, speed_percentage)
